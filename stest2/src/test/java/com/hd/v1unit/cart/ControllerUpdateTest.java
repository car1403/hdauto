...
@DisplayName(" Cart Controller Update Test ")
public class ControllerUpdateTest {
    ...
    @Test
    @Order(1)
    @DisplayName("Cart Update 정상")
    void success1() throws Exception {

        //given
        CartRequestDto requestDto =
                CartRequestDto.builder().id(1L).cnt(2L)
                        .custId("id01")
                        .itemId(1L)
                        .build();
        String body = new ObjectMapper().writeValueAsString(requestDto); // json 으로 변경한것


        //stub
        given(cartService.modify(any())).willReturn( CartEntity.builder().id(1L).cnt(3L)
                .custEntity(CustEntity.builder().id("id01").build())
                .itemEntity(ItemEntity.builder().id(1L).build())
                .build());

        //when
        ResultActions resultActions= mockMvc.perform(patch("/api/cart/update")
        ...
        resultActions
                ...
                .andExpect(jsonPath("$.data.cnt").value(3L))
                .andDo(print());
    }
    @Test
    @Order(2)
